...
public class CustomProjectiles : MonoBehaviour
{
    ...
    public void Explode()
    {
        ...
        {
            //Damage enemies
            if (enemies[i].GetComponent<BasicEnemy>())
    ...
    //    //Explode on touch
    //    if (explodeOnTouch && other.CompareTag("Enemy")) Explode();
    ...
    // OnTriggerEnter kullanma sebebim objelerin fiziksel temasta fiziksel etkileşime girmesini istemiyor olmam. Enemy fırlasın istemiyorum. Eğer düzeltmek istersen triger yerine OnCollisionEnter içinde if bloğunu aktif edebilirsin.
    // Bunun için isTrigger özelliğini kullandım.
